...
const APP_0001 = `CREATE TABLE \`account_state\` (
	\`id\` text PRIMARY KEY NOT NULL,
...
--> statement-breakpoint
CREATE UNIQUE INDEX \`account_state_identity_ns_key_idx\` ON \`account_state\` (\`identity_key\`,\`namespace\`,\`key\`);
--> statement-breakpoint
CREATE INDEX \`account_state_identity_idx\` ON \`account_state\` (\`identity_key\`);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX \`draft_attachments_identity_draft_attach_idx\` ON \`draft_attachments\` (\`identity_key\`,\`draft_id\`,\`attachment_id\`);
--> statement-breakpoint
...
export const MIGRATIONS: Migration[] = [
  ...
  { target: "app", name: "0001_burly_titania", sql: APP_0001 },
